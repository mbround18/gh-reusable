...
test("docker release workflow has rich summary and PR/release reporting steps", () => {
  ...
  const reportStep = steps.find(
    (step) => step.name === "Materialize docker release report",
  );
  const reportUpload = steps.find(
    (step) => step.name === "Upload docker release report artifact",
  );
  const stickyCommentStep = steps.find(
    (step) => step.name === "Update sticky PR status comment",
  );
  const historyStep = steps.find(
    (step) => step.name === "Add PR run history comment",
  );
  const releaseNotesStep = steps.find(
    (step) => step.name === "Update tag release notes",
  );
  const failureStep = steps.find(
    (step) => step.name === "Fail workflow on docker release error",
  );
  ...
  expect(stickyCommentStep?.env?.SUMMARY_JSON).toContain(
    "steps.dagger_release.outputs.stdout",
  );
  expect(historyStep?.uses).toContain("actions/github-script@");
  expect(historyStep?.env?.SUMMARY_JSON).toContain(
    "steps.dagger_release.outputs.stdout",
  );
  expect(releaseNotesStep?.if).toContain("inputs.track_release_summary");
  expect(releaseNotesStep?.env?.SUMMARY_JSON).toContain(
    "steps.dagger_release.outputs.stdout",
  );
