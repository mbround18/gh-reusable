...
test("audit workflow runs module audit and supports SARIF alerts + PR history", () => {
  ...
  const sarifStep = steps.find(
    (step) => step.name === "Generate SARIF reports for alerts",
  );
  const semgrepUpload = steps.find(
    (step) => step.name === "Upload Semgrep SARIF",
  );
  const gitleaksUpload = steps.find(
    (step) => step.name === "Upload Gitleaks SARIF",
  );
  const reportStep = steps.find(
    (step) => step.name === "Materialize audit report",
  );
  const reportUpload = steps.find(
    (step) => step.name === "Upload audit report artifact",
  );
  const stickyComment = steps.find(
    (step) => step.name === "Update sticky PR audit comment",
  );
  const historyComment = steps.find(
    (step) => step.name === "Add PR audit history comment",
  );
  const releaseNotes = steps.find(
    (step) => step.name === "Update tag release notes with audit summary",
  );
