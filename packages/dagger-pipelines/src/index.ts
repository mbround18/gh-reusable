...
export function withMountedSource(
  container: Container,
  sourceDir: Directory,
): Container {
...
export function runCommands(
  ...
  return commands.reduce(
    (current, command) => runCommand(current, command),
    container,
  );
}
...
export async function buildAndPush(
  ...
  if (config.docker) {
    ...
    const dockerContainer = buildDockerContainer(
      built.directory("."),
      metadata,
      workspacePath,
    );
...
function buildDockerContainer(
  ...
  const contextSubpath = toWorkspaceSubpath(
    workspacePath,
    metadata.contextAbsolute,
  );
  ...
  const shouldBuildFromWorkspace =
    dockerfileFromContext.startsWith("..") ||
    path.isAbsolute(dockerfileFromContext);

  if (shouldBuildFromWorkspace) {
    const workspaceDockerfile = normalizeSubpath(
      path.relative(workspacePath, metadata.dockerfileAbsolute),
    );
  ...
  const dockerBuildSource =
    contextSubpath === "." ? sourceDir : sourceDir.directory(contextSubpath);
...
function withPublishAuth(
  ...
  const authEntries = Array.isArray(publish.auth)
    ? publish.auth
    : [publish.auth];
  return authEntries.reduce((current, auth, index) => {
    ...
    const secretName =
      `${resolved.username}-${auth.address}-registry-auth-${index}`.replace(
        /[^a-zA-Z0-9_.-]/g,
        "-",
      );
...
function readDockerConfigAuth(
  ...
  if (config.credsStore) {
    for (const candidateKey of candidateKeys) {
      const credentials = readDockerCredentialHelper(
        config.credsStore,
        candidateKey,
      );
      if (credentials) {
