...
const pnpmPipelineTest = shouldSkipDaggerSmokeTests() ? test.skip : test;

pnpmPipelineTest(
  "runs the pnpm pipeline against the repo root",
  async () => {
...
pnpmPipelineTest(
  "rejects package.json files without build and test scripts",
  async () => {
    const binary = ensureDaggerBinary();
