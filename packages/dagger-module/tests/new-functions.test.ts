...
describe('inferIncrement — label-based', () => {
  test('picks major from label', () => {
    expect(inferIncrement('major', '', 'major', 'minor', 'patch')).toBe('major');
  });
  test('picks minor from label', () => {
    expect(inferIncrement('minor,other', '', 'major', 'minor', 'patch')).toBe('minor');
  });
  test('picks patch from label', () => {
    expect(inferIncrement('patch', '', 'major', 'minor', 'patch')).toBe('patch');
  });
  test('defaults to patch when no labels match', () => {
    expect(inferIncrement('unrelated-label', '', 'major', 'minor', 'patch')).toBe('patch');
  });
  test('labels take precedence over branch name', () => {
    expect(inferIncrement('minor', 'breaking/something', 'major', 'minor', 'patch')).toBe('minor');
  });
});

describe('inferIncrement — branch-based', () => {
  test('feat/ prefix → minor', () => {
    expect(inferIncrement('', 'feat/new-thing', 'major', 'minor', 'patch')).toBe('minor');
  });
  test('feature/ prefix → minor', () => {
    expect(inferIncrement('', 'feature/cool', 'major', 'minor', 'patch')).toBe('minor');
  });
  test('breaking/ prefix → major', () => {
    expect(inferIncrement('', 'breaking/api', 'major', 'minor', 'patch')).toBe('major');
  });
  test('breaking-change in name → major', () => {
    expect(inferIncrement('', 'my-breaking-change', 'major', 'minor', 'patch')).toBe('major');
  });
  test('fix/ prefix → patch', () => {
    expect(inferIncrement('', 'fix/bug', 'major', 'minor', 'patch')).toBe('patch');
  });
  test('chore/ prefix → patch', () => {
    expect(inferIncrement('', 'chore/cleanup', 'major', 'minor', 'patch')).toBe('patch');
  });
  test('release/ prefix → minor', () => {
    expect(inferIncrement('', 'release/1.2', 'major', 'minor', 'patch')).toBe('minor');
  });
  test('unmatched branch → patch default', () => {
    expect(inferIncrement('', 'my-random-branch', 'major', 'minor', 'patch')).toBe('patch');
  });
...
describe('dockerFacts — push decision', () => {
  const base = { defaultBranch: 'main', canaryLabel: 'canary', forcePush: false, prLabelsCsv: '' };

  test('push on default branch', () => {
    expect(resolveDockerFactsPush({ ...base, eventName: 'push', ref: 'refs/heads/main' })).toBe(true);
  });
  test('push on tag ref', () => {
    expect(resolveDockerFactsPush({ ...base, eventName: 'push', ref: 'refs/tags/v1.0.0' })).toBe(true);
  });
  test('no push on non-default branch', () => {
    expect(resolveDockerFactsPush({ ...base, eventName: 'push', ref: 'refs/heads/feature/x' })).toBe(false);
  });
  test('no push on PR without canary label', () => {
    expect(resolveDockerFactsPush({ ...base, eventName: 'pull_request', ref: 'refs/pull/1/merge' })).toBe(false);
  });
  test('push on PR with canary label', () => {
    expect(
      resolveDockerFactsPush({ ...base, eventName: 'pull_request', ref: 'refs/pull/1/merge', prLabelsCsv: 'canary,other' }),
    ).toBe(true);
  });
  test('force push always pushes', () => {
    expect(resolveDockerFactsPush({ ...base, eventName: 'pull_request', ref: 'refs/pull/1/merge', forcePush: true })).toBe(true);
  });
});
...
describe('graphql args parsing', () => {
  ...
  test('comma-separated pairs', () => {
    expect(parseArgs('owner=mbround18, repo=gh-reusable')).toEqual({ owner: 'mbround18', repo: 'gh-reusable' });
  });
  test('newline-separated pairs', () => {
    expect(parseArgs('owner=mbround18\nrepo=gh-reusable')).toEqual({ owner: 'mbround18', repo: 'gh-reusable' });
  });
