...
export class PipelineCache {
  ...
  static async create(inputs: CacheInputs): Promise<PipelineCache> {
    ...
    const lockfileHash = await hashMatchedFiles(
      inputs.source,
      inputs.lockfilePatterns,
    );
    const sourceHash = await hashMatchedFiles(
      inputs.source,
      inputs.sourcePatterns,
    );
    const commit = environment.GITHUB_SHA ?? "unknown-commit";
    const engine =
      environment.DAGGER_VERSION ??
      environment.DAGGER_ENGINE_VERSION ??
      "unknown-engine";
  ...
  async save(container: Container, shouldSave = true): Promise<SaveResult> {
    ...
    const exportRoot = await mkdtemp(
      path.join(os.tmpdir(), `${this.inputs.pipelineName}-cache-`),
    );
    try {
      for (const mount of this.inputs.mounts) {
      ...
      if (this.backend === "github") {
        try {
          ...
          warnings.push(
            `GitHub cache upload failed for ${this.cacheKey}: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
        return { warnings };
      }
  ...
  private async restoreArchive(warnings: string[]): Promise<string | null> {
    const extractRoot = await mkdtemp(
      path.join(os.tmpdir(), `${this.inputs.pipelineName}-restore-`),
    );
    try {
      if (this.backend === "github") {
        const restored = await actionsCache.restoreCache(
          [extractRoot],
          this.cacheKey,
          this.restoreKeys,
        );
      ...
      const tarball =
        this.backend === "github"
          ? path.join(os.tmpdir(), `${this.safeName(this.cacheKey)}.tar.gz`)
          : await this.downloadFromS3(warnings);
      if (!tarball) {
        await rm(extractRoot, { recursive: true, force: true });
        return null;
      }
      ...
      warnings.push(
        `Cache restore failed for ${this.cacheKey}: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    } finally {
      await rm(extractRoot, { recursive: true, force: true });
  ...
  private async downloadFromS3(warnings: string[]): Promise<string | null> {
    ...
    if (!head.ok) {
      warnings.push(
        `S3 cache lookup failed for ${this.cacheKey}: ${head.status} ${head.statusText}`,
      );
      return null;
    }
    ...
    if (!get.ok) {
      warnings.push(
        `S3 cache download failed for ${this.cacheKey}: ${get.status} ${get.statusText}`,
      );
      return null;
    }
    ...
    const tarball = path.join(
      os.tmpdir(),
      `${this.safeName(this.cacheKey)}.tar.gz`,
    );
  ...
  private async uploadToS3(
    ...
    if (!put.ok) {
      warnings.push(
        `S3 cache upload failed for ${this.cacheKey}: ${put.status} ${put.statusText}`,
      );
    }
    await rm(tarball, { force: true });
  }
  ...
  private bucket(): string {
    return (
      (this.inputs.environment ?? process.env).S3_BUCKET?.trim() ||
      "dagger-cache"
    );
  }
  ...
  private region(): string {
    return (
      (this.inputs.environment ?? process.env).S3_REGION?.trim() || "us-east-1"
    );
  }
...
async function hashMatchedFiles(
  directory: Directory,
