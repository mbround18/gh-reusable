...
import { join, resolve, relative, normalize, isAbsolute, basename, dirname } from 'node:path';
import { execSync } from 'node:child_process';

const getInput = (n) => (process.env[`INPUT_${n.toUpperCase().replace(/-/g, '_')}`] ?? '').trim();
const setOutput = (n, v) => appendFileSync(process.env.GITHUB_OUTPUT, `${n}=${v}\n`);
...
function parseBuildArgs(args) {
  if (args && typeof args === 'object' && !Array.isArray(args)) {
    return Object.fromEntries(Object.entries(args).filter(([, v]) => v != null).map(([k, v]) => [k, String(v)]));
  }
  if (Array.isArray(args)) {
    return Object.fromEntries(
      args.filter(v => typeof v === 'string' && v.includes('=')).map(pair => {
...
function parseDockerCompose(filePath, imageName) {
  ...
  if (!compose?.services) return { dockerfile: null, context: null, buildArgs: {}, target: null };
  for (const service of Object.values(compose.services)) {
    if (typeof service.image !== 'string' || !service.image.startsWith(`${imageName}:`)) continue;
    const build = service.build;
    if (typeof build === 'string') return { dockerfile: null, context: build, buildArgs: {}, target: null };
...
function shouldPush({ eventName, ref, defaultBranch, canaryLabel, forcePush, eventPath }) {
  if (forcePush) return true;
  if (ref === `refs/heads/${defaultBranch}` || ref.startsWith('refs/tags/')) return true;
  if (eventName === 'pull_request') {
    try {
      const event = JSON.parse(readFileSync(eventPath, 'utf8'));
      const labels = (event?.pull_request?.labels ?? []).map(l => l?.name).filter(Boolean);
...
function generateTags({ image, version, registries, withLatest, ref, target, prependTarget }) {
  const prefix = prependTarget && target ? `${target}-` : '';
  const vTag = version.startsWith('v') ? `${prefix}${version}` : `${prefix}v${version}`;
...
try {
  ...
  const registries = getInput('registries').split(',').map(s => s.trim()).filter(Boolean);
  ...
  const defaultBranch = process.env.GITHUB_BASE_REF ?? process.env.GITHUB_DEFAULT_BRANCH ?? 'main';
  ...
  if (composeFile) {
    ...
    if (compose.dockerfile) {
      const composeCtxRel = compose.context ? join(contextInput, compose.context) : null;
      if (composeCtxRel) {
        ctx = resolvePath(workspace, composeCtxRel);
        dockerfile = findDockerfile(workspace, join(composeCtxRel, compose.dockerfile), ctx);
      } else {
        dockerfile = findDockerfile(workspace, join(contextInput, compose.dockerfile), ctx);
      }
  ...
  const push = shouldPush({ eventName, ref, defaultBranch, canaryLabel, forcePush, eventPath });
  const tags = generateTags({ image, version, registries, withLatest, ref, target, prependTarget });
