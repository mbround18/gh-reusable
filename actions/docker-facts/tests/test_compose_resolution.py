...
class TestDockerComposeResolution(unittest.TestCase):
    ...
    def test_find_docker_compose_in_workspace_root(self, mock_resolve, mock_workspace):
        ...
        mock_resolve.side_effect = lambda path, to_relative=False: os.path.join(self.workspace, path) if path and not os.path.isabs(path) else path
    ...
    def test_find_docker_compose_in_context_dir(self, mock_resolve, mock_workspace):
        ...
        self.assertEqual(result, os.path.join(self.workspace, "app/docker-compose.yaml"))
    ...
    @patch('index.get_github_workspace')
    @patch('index.resolve_path')
    def test_search_alternative_filenames_in_workspace(self, mock_resolve, mock_workspace):
