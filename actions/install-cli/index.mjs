import { existsSync, mkdirSync, readdirSync, renameSync, chmodSync, appendFileSync, createWriteStream } from 'node:fs';
...
async function resolveVersion(owner, repo, version, token) {
  ...
  if (!res.ok) throw new Error(`Failed to fetch latest release for ${owner}/${repo}: ${res.status} ${res.statusText}`);
  const data = await res.json();
  if (!data.tag_name) throw new Error(`No tag_name found in latest release for ${owner}/${repo}`);
...
async function run() {
  try {
    ...
    const cacheRoot = join(process.env.RUNNER_TOOL_CACHE ?? '/tmp/tool-cache', `${owner}-${repo}-${resolvedVersion}`);
    ...
    if (existsSync(cacheRoot) && readdirSync(cacheRoot).length > 0) {
      ...
      if (overrideName) {
        ...
        const binary = entries.find(e => {
          try { return !existsSync(join(cacheRoot, e, '.')); } catch { return true; }
        });
