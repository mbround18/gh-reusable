...
const LOCKFILES = ['pnpm-lock.yaml', 'yarn.lock', 'package-lock.json', 'Cargo.lock'];
const MANIFEST_FILENAMES = ['package.json', 'Cargo.toml', 'pyproject.toml', 'requirements.txt', 'go.mod', 'Chart.yaml'];
...
try {
  ...
  if (hasPnpm) { packageManagers.push('pnpm'); nodePackageManager = 'pnpm'; }
  else if (hasYarn) { packageManagers.push('yarn'); nodePackageManager = 'yarn'; }
  else if (hasNpm || hasPackageJson) { packageManagers.push('npm'); nodePackageManager = 'npm'; }
  ...
  appendFileSync(process.env.GITHUB_OUTPUT, `json<<MATRIX\n${jsonStr}\nMATRIX\n`);
